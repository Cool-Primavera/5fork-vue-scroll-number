      ...
      <ul class="number-list padding-number-list">
        ...
        >
          <div class="number-item" :style="itemStyle">
            <span>{{
              numbers[index] + numbers.length - backwardPaddingNumbers.length
            }}</span>
          </div>
        </li>
      </ul>
...
export default {
  ...
  computed: {
    ...
    listStyle() {
      const { backwardPaddingNumbers, currentIndex, itemHeight } = this;
      return {
  ...
  methods: {
    ...
    backwardTo(newValue) {
      const p = () =>
        new Promise((resolve) => {
          ...
          // 当新数比原来大，往后倒
          if (diff > 0) {
            this.backwardPaddingNumbers = [
              ...new Array(this.numbers.length - newValue).keys(),
            ];
    ...
    calcItemSize() {
      this.itemSize = this.$refs.realList
        .querySelector("span")
